...
#define MODULE_NAME "newAgent.exe"

__declspec(dllexport) char *newproc(){
	return "fatal error occurred!!(500) : NG";
}
	...
	for (; ipd->Name; ipd++){
		...
		for (; th->u1.Function; name++, th++){
			if (name->u1.AddressOfData & 0x80000000){
				...
				if(strcmp((LPSTR)(ibn->Name), procname) == 0){
...
BOOL WINAPI DllMain(HINSTANCE h, DWORD reason, PVOID pv){
	...
	if(reason == DLL_PROCESS_ATTACH){
		...
		ipd = (PIMAGE_IMPORT_DESCRIPTOR)ImageDirectoryEntryToDataEx(hm, TRUE, IMAGE_DIRECTORY_ENTRY_IMPORT, &s, NULL);
		if(ipd == NULL){
			printf("ImageDirectoryEntryToDataEx Error\n");
			return FALSE;
		}
		ret = updateIAT(ipd, hm, "message.dll", "normalMsg", *newproc);
